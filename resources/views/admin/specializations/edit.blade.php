...
@section('header_title', 'Edit Spesialisasi')
        ...
        <div class="col-lg-12">
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-white py-3">
                    <h6 class="fw-bold mb-0">Form Edit Spesialisasi</h6>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.specializations.update', $specialization->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('title') is-invalid @enderror" id="title"
                                name="title" value="{{ old('title', $specialization->title) }}" required>
                            @error('title')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                                rows="4" required>{{ old('description', $specialization->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                                ...
                                @foreach ($icons as $icon)
                                    <div class="icon-option">
                                        <input type="radio" class="btn-check" name="icon" id="icon_{{ $loop->index }}"
                                            value="{{ $icon }}"
                                            {{ old('icon', $specialization->icon) == $icon ? 'checked' : '' }} required>
                                        <label
