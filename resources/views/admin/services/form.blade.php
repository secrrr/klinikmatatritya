...
@section('content')
<h4>{{ $item->exists ? 'Edit' : 'Create' }} Service</h4>
<form method="post" enctype="multipart/form-data" action="{{ $item->exists ? route('admin.services.update',$item->id) : route('admin.services.store') }}">@csrf @if($item->exists) @method('PUT') @endif
